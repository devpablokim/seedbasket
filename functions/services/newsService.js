...
const NEWS_API_KEY = functions.config().newsapi?.key || process.env.NEWS_API_KEY;

async function fetchFromNewsAPI() {
  try {
    const response = await axios.get('https://newsapi.org/v2/everything', {
      params: {
        q: '(ETF OR "exchange traded fund") AND (market OR economy OR inflation OR "federal reserve" OR earnings OR commodity)',
        apiKey: NEWS_API_KEY,
        language: 'en',
        sortBy: 'publishedAt',
        pageSize: 30,
...
module.exports = {
  fetchLatestNews,
  categorizeNews,
  scheduledNewsUpdate: exports.scheduledNewsUpdate
};