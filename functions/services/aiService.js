...
// Initialize OpenAI client lazily to avoid deployment errors
let openai;
function getOpenAIClient() {
  if (!openai) {
    const apiKey = functions.config().openai?.key || functions.config().openai?.api_key || process.env.OPENAI_API_KEY || 'placeholder';
